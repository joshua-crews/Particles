...
void validate_pixel_contribs(
    const Particle *cuda_particles, const unsigned int particles_count,
    ...
    skip_pixel_contribs(cuda_particles, particles_count, pixel_contribs, out_image_width, out_image_height);
...
void validate_sorted_pairs(
    ...
    skip_sorted_pairs(
        cuda_particles, particles_count,
        ...
        for (int x = x_min; x <= x_max; ++x) {
            for (int y = y_min; y <= y_max; ++y) {
                ...
                if (pixel_distance <= cuda_particles[i].radius) {
                    ...
                    memcpy(return_pixel_contrib_colours + (4 * storage_offset), cuda_particles[i].color, 4 * sizeof(unsigned char));
                    memcpy(return_pixel_contrib_depth + storage_offset, &cuda_particles[i].location[2], sizeof(float));
                }
            }
        }
    }
