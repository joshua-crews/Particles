...
int main(int argc, char **argv)
{
    ...
    Particle* cuda_particles = (Particle *)malloc(particles_count * sizeof(Particle));
    {
        ...
        for (unsigned int i = 0; i < config.circle_count; ++i) {
            ...
            cuda_particles[i].color[0] = base_color_palette[palette_index][0];
            cuda_particles[i].color[1] = base_color_palette[palette_index][1];
            cuda_particles[i].color[2] = base_color_palette[palette_index][2];
            cuda_particles[i].location[0] = normalised_float_dist(rng) * config.out_image_width;
            cuda_particles[i].location[1] = normalised_float_dist(rng) * config.out_image_height;
            cuda_particles[i].location[2] = depths[i];
            // Circle specific
            cuda_particles[i].radius = circle_rad_dist(rng);
            ...
            cuda_particles[i].color[3] = (unsigned char)(255 * t_opacity);
        }
...
#pragma omp parallel for 
        for (int i = 0; i < (int)particles_count; ++i) {
            cuda_particles[i].radius = cuda_particles[i].radius < MIN_RADIUS ? MIN_RADIUS : cuda_particles[i].radius;
            cuda_particles[i].radius = cuda_particles[i].radius > MAX_RADIUS ? MAX_RADIUS : cuda_particles[i].radius;
        }
    }
    ...
    CImage validation_image;
    {
        ...
        // Run algorithm
        skip_pixel_contribs(cuda_particles, particles_count, pixel_contribs, validation_image.width, validation_image.height);
        ...
        skip_sorted_pairs(cuda_particles, particles_count, pixel_index, validation_image.width, validation_image.height, pixel_contrib_colours, pixel_contrib_depth);
    ...
    const int TOTAL_RUNS = config.benchmark ? BENCHMARK_RUNS : 1;
    {
        ...
        for (int runs = 0; runs < TOTAL_RUNS; ++runs) {
            ...
            case CPU:
                {
                    cpu_begin(cuda_particles, particles_count, config.out_image_width, config.out_image_height);
            ...
            case OPENMP:
                {
                    openmp_begin(cuda_particles, particles_count, config.out_image_width, config.out_image_height);
                    CUDA_CALL(cudaEventRecord(initT));
                    CUDA_CALL(cudaEventSynchronize(initT));
                    openmp_stage1();
                    CUDA_CALL(cudaEventRecord(stage1T));
            ...
            case CUDA:
                {
                    cuda_begin(cuda_particles, particles_count, config.out_image_width, config.out_image_height);
                    CUDA_CHECK();
                    CUDA_CALL(cudaEventRecord(initT));
                    CUDA_CALL(cudaEventSynchronize(initT));
                    cuda_stage1();
